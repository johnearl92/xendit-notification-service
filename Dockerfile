...
ENV CGO_ENABLED=0 \
    ...
    GOARCH=amd64
ARG VERSION=dev
WORKDIR /go/src/github.com/johnearl92/xendit-notification-service
COPY go.* ./
COPY . .
# RUN go build -mod vendor -o /go/bin/xendit-notification-service -ldflags "-X main.version=${VERSION} -w -s"
RUN GO111MODULE=on go build -mod=vendor -o /go/bin/xendit-notification-service -ldflags "-X main.version=${VERSION} -w -s"
...
COPY --from=build /go/bin/xendit-notification-service /xendit-notification-service
...
##
ENTRYPOINT [ "/xendit-notification-service" ]
CMD [ "serve" ]