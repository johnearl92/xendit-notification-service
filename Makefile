...
#################################################################
DOCKER_REGISTRY=
APP=xendit-notification-service
VERSION=latest
TAG=latest
PROJECT=jagbay01
IMAGE=${DOCKER_REGISTRY}${PROJECT}/${APP}:${TAG}

NAMESPACE=xendit-notification-service
HELM_CHART=xendit-notification-service
VALUES_FILE=helm/xendit-notification-service/values/dev.yaml
...
deploy-helm:
	helm upgrade ${HELM_CHART} --install \
		...
		--values ${VALUES_FILE} helm/xendit-notification-service
	@echo 'Done.'
